...
import 'package:wallpaper/ui/widgets/menuButton/favIconButton.dart';
import 'package:wallpaper/ui/widgets/premiumBanners/followingFeed.dart';
...
class _FollowingScreenState extends State<FollowingScreen> {
    ...
    _streamController!.stream.listen((p) {
      setState(() {
        ...
        for (final doc in finalQuery.docs) {
          if (following.contains(doc.get("email")) && finalDocs.length <= 30) {
            finalDocs.add(doc);
          }
        }
      });
  ...
  Future<void> load(StreamConsumer<QuerySnapshot<Map<String, dynamic>>> sc) async {

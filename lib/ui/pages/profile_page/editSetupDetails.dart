...
import 'package:wallpaper/data/apps/appsData.dart';
...
import 'package:wallpaper/data/upload/wallpaper/wallfirestore.dart' as WallStore;
...
class _EditSetupReviewScreenState extends State<EditSetupReviewScreen> {
  ...
  void initState() {
    ...
    Map<String,dynamic> data=setupDoc as Map<String, dynamic>;
    ...
    id = data["id"].toString();
    setupDesc =
        TextEditingController(text: data["desc"].toString());
    iconName = TextEditingController(text: data["icon"].toString());
    iconURL =
        TextEditingController(text: data["icon_url"].toString());
    widgetName1 =
        TextEditingController(text: data["widget"].toString());
    widgetURL1 =
        TextEditingController(text: data["widget_url"].toString());
    if ("${data["wallpaper_url"]}" != "") {
      if ("${data["wallpaper_url"]}"[0] != "[") {
        if ("${data["wall_id"]}" != "" &&
            "${data["wall_id"]}" != null) {
          wallpaperUploaded = true;
          wallpaperUploadLink = data["wallpaper_url"].toString();
          wallpaperId = data["wall_id"].toString();
          groupValue = 1;
        } else {
          wallpaperUrl = TextEditingController(
              text: data["wallpaper_url"].toString());
          groupValue = 0;
        }
      } else {
        wallpaperAppName = TextEditingController(
            text: data["wallpaper_url"][0].toString());
        wallpaperAppWallName = TextEditingController(
            text: data["wallpaper_url"][2].toString());
        wallpaperAppLink = TextEditingController(
            text: data["wallpaper_url"][1].toString());
        groupValue = 2;
      }
    } else {
      wallpaperUrl = TextEditingController(
    ...
    widgetName2 =
        TextEditingController(text: data["widget2"].toString());
    widgetURL2 =
        TextEditingController(text: data["widget_url2"].toString());
    isUploading = false;
    isProcessing = false;
    wallpaperProvider = data["wallpaper_provider"].toString();
    wallpaperThumb = data["wallpaper_thumb"].toString();
    review = data["review"] as bool?;
    secondWidgetAdded = "${data["widget2"]}" != "" &&
        "${data["widget2"]}" != null;
  }
